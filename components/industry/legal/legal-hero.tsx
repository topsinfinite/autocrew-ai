    ...
    <section className="relative z-10 section-glow-bottom overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4 sm:pt-5 md:pt-6 pb-4 sm:pb-6 relative z-10">
        <div className="text-center max-w-4xl mt-4 sm:mt-6 mx-auto">
          ...
          >
            <SectionBadge>{legalHeroData.badge}</SectionBadge>
          </div>
          ...
          >
            <span className="block">{legalHeroData.headline.prefix}</span>
          ...
          >
            <Button
              ...
              asChild
            >
              <Link href={legalHeroData.primaryCta.href}>
                {legalHeroData.primaryCta.text}
                <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </Link>
            </Button>

            <Link
              href={legalHeroData.secondaryCta.href}
              className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              <PlayCircle className="w-4 h-4 text-[#FF6B35]" />
              {legalHeroData.secondaryCta.text}
            </Link>
          </div>
          ...
          >
            <span className="font-geist">{legalHeroData.trustText}</span>
          </div>
        </div>
      </div>
      ...
      >
        <div className="relative">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-0">
            {legalHeroData.workflowSteps.map((step, index) => {
              const Icon =
                workflowIcons[step.icon as keyof typeof workflowIcons];
              const isLast = index === legalHeroData.workflowSteps.length - 1;
