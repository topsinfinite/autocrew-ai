...
import { coachingAskStrip } from "@/lib/mock-data/coaching-data";

/**
 * In-context "Ask Sarah" strip — sparing mid-page surface that lets the
 * visitor poke the live widget with a coaching-flavoured question without
 * leaving the page.
 */
export function CoachingAskStrip() {
  return (
          ...
          <div className="lg:col-span-7">
            <div className="rounded-2xl border border-[var(--border-subtle)] bg-foreground/[0.02] p-5 sm:p-7">
              <ul className="grid gap-2.5 sm:grid-cols-2">
                {coachingAskStrip.prompts.map((p) => (
                  <li key={p.label}>
                    <button
                      type="button"
