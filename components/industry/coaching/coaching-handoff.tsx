import { coachingHandoff } from "@/lib/mock-data/coaching-data";

/**
 * Handoff rules ledger — when Sarah hands the conversation back to the
 * coach. Plain three-column tabular layout.
 */
export function CoachingHandoff() {
  return (
          ...
          <div className="lg:col-span-7">
            <div className="overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-card">
              <div className="hidden grid-cols-12 gap-4 border-b border-[var(--border-subtle)] bg-foreground/[0.025] px-5 py-3 lg:grid">
                {coachingHandoff.columns.map((col, i) => (
                  <div
                    key={col}
                    className={
              ...
              <ul className="divide-y divide-[var(--border-subtle)]">
                {coachingHandoff.rows.map((row, i) => (
                  <li
                    key={row.condition}
