import { healthcareByRole } from "@/lib/mock-data/healthcare-data";

/**
 * "Three answers, in the language of three jobs."
 *
 * For the CFO, CMO, and practice owner. Three editorial columns of equal
 * weight — paragraph + a single mono outcome line. No icons, no card stack
 * theatre.
 */
export function HealthcareByRole() {
  return (
    <section className="relative z-10 border-t border-[var(--border-subtle)]">
      <div className="mx-auto max-w-[1320px] px-6 py-20 lg:py-28">
        ...
        <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--border-subtle)] sm:grid-cols-3">
          {healthcareByRole.roles.map((role) => (
            <article
              key={role.title}
