...
import { healthcareCompliance } from "@/lib/mock-data/healthcare-data";

/**
 * Compliance ledger — translates the FHIR / SMART / AES / JWT material from
 * the technical docs into language a CFO or practice owner can take into a
 * conversation with their security officer. A single horizontal mono ledger
 * does the work that a forest of green-checkmark cards usually attempts.
 */
export function HealthcareCompliance() {
  return (
        ...
        <div className="grid gap-10 lg:grid-cols-12 lg:gap-16">
          <div className="lg:col-span-5">
            ...
            <h2 className="mt-4 font-space-grotesk text-[clamp(1.875rem,3vw,2.75rem)] font-semibold leading-[1.08] tracking-[-0.018em] text-foreground">
              {healthcareCompliance.heading}
            </h2>
            <p className="mt-5 max-w-[44ch] font-geist text-[15px] leading-[1.7] text-foreground/70">
              {healthcareCompliance.body}
            </p>
            <Link
              href={healthcareCompliance.docsLink.href}
            ...
            >
              {healthcareCompliance.docsLink.text}
          ...
          <div className="lg:col-span-7">
            <dl className="grid grid-cols-2 divide-x divide-y divide-[var(--border-subtle)] overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-card md:grid-cols-3">
              {healthcareCompliance.ledger.map((cell) => (
                <div key={cell.label} className="px-5 py-6 sm:px-6 sm:py-7">
                  ...
                  <dd className="mt-2 font-space-grotesk text-[clamp(1.125rem,1.6vw,1.375rem)] font-semibold leading-[1.2] tracking-[-0.01em] text-foreground">
                    {cell.value}
                  </dd>
                </div>
              ))}
