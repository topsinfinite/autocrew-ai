import { healthcareHandoff } from "@/lib/mock-data/healthcare-data";

/**
 * Handoff rules ledger — when the AI knows to stop and put a human on.
 * A plain three-column tabular layout in mono microcopy + body text. No
 * green-checkmark theatre, no border-left accents.
 */
export function HealthcareHandoff() {
  return (
          ...
          <div className="lg:col-span-7">
            <div className="overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-card">
              <div className="hidden grid-cols-12 gap-4 border-b border-[var(--border-subtle)] bg-foreground/[0.025] px-5 py-3 lg:grid">
                {healthcareHandoff.columns.map((col, i) => (
                  <div
                    key={col}
                    className={
              ...
              <ul className="divide-y divide-[var(--border-subtle)]">
                {healthcareHandoff.rows.map((row, i) => (
                  <li
                    key={row.condition}
