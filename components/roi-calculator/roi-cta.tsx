...
import { roiCalculatorCta } from "@/lib/mock-data/roi-calculator-data";
import { footerData } from "@/lib/mock-data/landing-data";

const badgeIcon = { Conservative: Plug, Setup: Settings2 };

/**
 * ROI calculator CTA + footer. Mirrors the coaching CTA pattern: dark
 * gradient card, oversized headline, three-column contact grid, brand
 * block + nav columns, bottom legal strip.
 */
        ...
        <div className="group relative overflow-hidden rounded-[40px] border border-border bg-card p-6 text-card-foreground shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)] transition-colors duration-500 sm:p-10 dark:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)]">
          ...
          <div className="relative z-10">
            ...
            <ul className="mb-6 flex flex-wrap gap-2 sm:mb-8">
              {roiCalculatorCta.badges.map((b) => {
                const Icon = b.label.startsWith("Setup")
                  ? badgeIcon.Setup
                  : badgeIcon.Conservative;
                return (
                  <li
                    key={b.label}
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 sm:mt-8 sm:gap-10 md:grid-cols-3">
              <div className="flex flex-col items-start">
                <p className="mb-3 font-space text-xs font-semibold uppercase tracking-wider text-[#FF6B35]">
                  {roiCalculatorCta.contact.email.eyebrow}
                </p>
                <a
                  href={`mailto:${roiCalculatorCta.contact.email.address}`}
                ...
                >
                  ...
                  <span className="break-all">
                    {roiCalculatorCta.contact.email.address}
                  </span>
                </a>
              </div>
              ...
              <div className="flex flex-col items-start">
                <p className="mb-3 font-space text-xs font-semibold uppercase tracking-wider text-[#FF6B35]">
                  {roiCalculatorCta.contact.demo.eyebrow}
                </p>
                <Button
                  variant="pill"
                  size="pill-md"
                  ...
                  asChild
                >
                  <Link href={roiCalculatorCta.contact.demo.cta.href}>
                    <Calendar className="h-4 w-4" />
                    {roiCalculatorCta.contact.demo.cta.text}
              ...
              <div className="flex flex-col items-start">
                <p className="mb-3 font-space text-xs font-semibold uppercase tracking-wider text-[#FF6B35]">
                  {roiCalculatorCta.contact.try.eyebrow}
                </p>
                <div className="flex w-full flex-col gap-4 sm:w-auto">
                  ...
                  >
                    ...
                    {roiCalculatorCta.contact.try.voiceCta.text}
                    <ArrowRight className="h-4 w-4 transition-transform group-hover/btn:translate-x-0.5" />
                  </Button>
                  <Link
                    href={roiCalculatorCta.contact.try.memberLink.href}
